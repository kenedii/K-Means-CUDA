...
{
    extern __shared__ int local_cluster_counts[]; // Shared memory for cluster counts

    int i = blockIdx.x * blockDim.x + threadIdx.x; // Get the sample index
    int tid = threadIdx.x;                         // Thread index

    // Step 1: Zero out shared cluster counts
    for (int c = tid; c < k_clusters; c += blockDim.x)
    {
        local_cluster_counts[c] = 0;
    }
    __syncthreads(); // Make sure everyone is done zeroing
    ...
    if (i < n_samples)
    {
        ...
        atomicAdd(&local_cluster_counts[cluster_idx], 1); // Increment the cluster count
    }

    __syncthreads(); // Wait for everyone to finish

    // Step 4: One thread per cluster flushes shared counts into global memory
    for (int c = tid; c < k_clusters; c += blockDim.x)
    {
        atomicAdd(&cluster_counts[c], local_cluster_counts[c]);
    }
}
